...
export const SmartAccountProvider: React.FC<SmartAccountProviderProps> = ({
  ...
  const bundlerClient = createPimlicoBundlerClient({
    transport: http(
      `https://api.pimlico.io/v1/base-sepolia/rpc?apikey=${process.env.NEXT_PUBLIC_PIMLICO_API_KEY}`
    ),
  });
    ...
    try {
      ...
      toast(`NFT minted successfully`, {
        action: {
          ...
          onClick: () =>
            (window.location.href = `https://sepolia.basescan.org/tx/${mint}`),
        },
      });
    } catch (err) {
    ...
    try {
      ...
      toast(`Transaction sent successfully`, {
        action: {
          label: "View on Polygon Scan",
          onClick: () =>
            (window.location.href = `https://sepolia.basescan.org/tx/${tx}`),
        },
      });
    } catch (error) {
